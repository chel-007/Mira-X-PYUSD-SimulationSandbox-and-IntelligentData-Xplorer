...
const SwapVolumeChart: React.FC<SwapVolumeChartProps> = ({ swapVolumeData, onDownload }) => {
  ...
  const renderChart = (swapVolumeData: SwapVolumeData[], width: number, height: number, animate: boolean) => {
    ...
    // Around line 110 (barData using color)
    ...
    const barData = dates.map((date, i) => ({
      ...
      segments: sortedPoolAddresses.map((pool, j) => {
        ...
        return {
          ...
          color: color(pool), // Ensure color is used here
          dateIndex: i,
        };
      }),
    }));
    ...
    // Create a single tooltip at the top level, after all bar groups
    ...
    // Attach events to rects after tooltip is created
    barGroupsEnter.each(function(d) {
      const group = d3.select(this);
      const rects = group.selectAll("rect");
      ...
      rects
        .on("mouseover", (event, seg) => {
          ...
          const text = `${seg.pool.slice(0, 4)}...${seg.pool.slice(-4)}: ${formatVolume(seg.rawVolume)} on ${d.date}`;
          ...
          tooltip
            .attr("transform", `translate(${x(parseDate(d.date)) + barWidth / 2},${Math.max(y(seg.y1), 0) - 10})`)
            .style("opacity", 1);
        })
        .on("mousemove", (event, seg) => {
          tooltip.attr("transform", `translate(${x(parseDate(d.date)) + barWidth / 2},${Math.max(y(seg.y1), 0) - 10})`);
        })
        .on("mouseout", (event) => {
    ...
    const logMin = Math.floor(Math.log10(minNonZeroVolume)); // Start below min
    const logMax = Math.ceil(Math.log10(maxStackedValue)) + 1; // Buffer above
// Update y-axis
...
// Mock data for descriptions and links (replace with your actual data)
const poolDetails = {
  [sortedPoolAddresses[0]]: { description: "(py/crv)", link: "https://curve.fi/dex/ethereum/pools/factory-stable-ng-42/deposit/" },
  [sortedPoolAddresses[1]]: { description: "(uniswap)", link: "https://etherscan.io/address/0xDd2e0D86A45e4EF9bd490c2809E6405720cC357c#events" },
  [sortedPoolAddresses[2]]: { description: "(PayPool)", link: "https://curve.fi/dex/ethereum/pools/factory-stable-ng-43/deposit/" },
};
...
addressText
  .on("mouseover", (event, d) => {
    d3.select(event.currentTarget).attr("fill", colors.hover || "#FFD700"); // Fallback to gold if colors.hover undefined
  })
  .on("mouseout", (event, d) => {
