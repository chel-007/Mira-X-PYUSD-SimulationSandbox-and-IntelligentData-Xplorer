...
const WalletGrowthChart: React.FC<WalletGrowthChartProps> = ({ walletGrowthData }) => {
  const growthRef = useRef<SVGSVGElement | null>(null);
  const chartContainerRef = useRef<HTMLDivElement>(null);
  const isFirstMount = useRef(true);
  const initialAnimationDone = useRef(false); // Track if initial animation has completed
  ...
  const renderChart = (svg, width, height, data, animate) => {
    const margin = { top: 20, right: 20, bottom: 30, left: 40 };

    // Update gradient
    updateGradient(svg, height, margin);

    const cumulativeData = data.reduce((acc, d, i) => {
    ...
    if (animate) {
      ...
      svg.selectAll(".dot").remove(); // Clear any existing dots
      svg.selectAll(".dot")
        ...
        .attr("r", 4)
        .on("end", () => {
          initialAnimationDone.current = true; // Mark animation complete
        });
    } else {
      // Non-animated update
      svg.select(".area")
  ...
  useEffect(() => {
    ...
    if (isFirstMount.current) {
      ...
      svg.append("path").attr("class", "area").attr("fill", "url(#areaGradient)");
      ...
      renderChart(svg, width, height, walletGrowthData, true);
      isFirstMount.current = false;
    } else if (initialAnimationDone.current) {
      // Only update if initial animation is done
      renderChart(svg, width, height, walletGrowthData, false);
    }

    // Handle resize
    const resizeObserver = new ResizeObserver(() => {
      if (chartContainerRef.current && growthRef.current && initialAnimationDone.current) {
        const newWidth = chartContainerRef.current.clientWidth;
        const newHeight = chartContainerRef.current.clientHeight;
        renderChart(svg, newWidth, newHeight, walletGrowthData, false);
      }
    });
